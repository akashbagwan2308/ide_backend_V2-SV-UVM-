# LogicSilicon Unified API: Verilator 5 Integration

This update replaces Icarus Verilog with Verilator 5. By utilizing the `--binary` and `--timing` flags, the backend now behaves exactly like a traditional event-driven simulator. This strict IEEE-1800 compliant environment ensures that code written in the playground adheres to industry standards, providing a rigorous foundation for Design Verification curricula. Furthermore, because Verilator transpiles to C++ under the hood, the generated binaries can be deeply instrumented, laying the necessary groundwork for future integrations with reinforcement learning and ML models to achieve real-time coverage closure.

## 1. `Dockerfile`
This Dockerfile builds Verilator 5 from the official source repository to ensure the latest SystemVerilog features (like `--timing`) are available.
...
## 2. `server.js`
The backend execution engine has been updated. The `/run` endpoint now utilizes Verilator to compile the `.sv` code into a C++ binary and subsequently executes it. 
...
## 3. `package.json`
The dependencies remain unchanged, but the description has been updated to reflect the new architecture.

```json
{
  "name": "logicsilicon-unified-api",
  "version": "3.0.0",
  "description": "Unified API for Verilator 5 Simulation and Yosys Synthesis",
  "main": "server.js",
  "scripts": {
